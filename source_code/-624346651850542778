...
contract Gethereum {
    ...
    struct UserStruct {
        bool isExist;
        uint256 id;
        uint256 referrerIDMatrix1;
        uint256 referrerIDMatrix2;
        address[] referralMatrix1;
        address[] referralMatrix2;
        uint256 referralCounter;
        mapping(uint256 => uint256) levelExpiredMatrix1;
        mapping(uint256 => uint256) levelExpiredMatrix2;
    }

    //A person can have maximum 2 branches
    uint256 private constant REFERRER_1_LEVEL_LIMIT = 2;
    //period of a particular level
    uint256 private constant PERIOD_LENGTH = 60 days;
    //person where the new user will be joined
    uint256 public availablePersonID;
    ...
    function() external payable {
        ...
        if (users[msg.sender].isExist) buyLevelMatrix1(level);
        else if (level == 1) {
            uint256 refId = 0;
    ...
    */
    function regUser(uint256 _referrerID) public payable {
        ...
        uint256 _referrerIDMatrix1;
        uint256 _referrerIDMatrix2 = _referrerID;

        _referrerIDMatrix1 = findAvailablePersonMatrix1();

        if (
            users[userList[_referrerIDMatrix2]].referralMatrix2.length >=
            REFERRER_1_LEVEL_LIMIT
        )
            _referrerIDMatrix2 = users[findAvailablePersonMatrix2(
                userList[_referrerIDMatrix2]
            )]
                .id;
    ...
    */
        ...
        for (uint256 i = 0; i < _adminAddress.length; i++) {
            ...
            if (currUserID == 1) {
                ...
                for (uint256 j = 1; j <= 6; j++) {
                    users[_adminAddress[i]]
                        .levelExpiredMatrix1[j] = 66666666666;
                    users[_adminAddress[i]]
                        .levelExpiredMatrix2[j] = 66666666666;
                }

            } else {
                uint256 _referrerIDMatrix1;
                uint256 _referrerIDMatrix2 = 1;

                _referrerIDMatrix1 = findAvailablePersonMatrix1();

                if (
                    users[userList[_referrerIDMatrix2]]
                        .referralMatrix2
                        .length >=
                    REFERRER_1_LEVEL_LIMIT
                )
                ...
                users[userList[_referrerIDMatrix1]].referralMatrix1.push(
                    _adminAddress[i]
                );
                users[userList[_referrerIDMatrix2]].referralMatrix2.push(
                    _adminAddress[i]
                );
    ...
    function addShareHolder(address[] memory _shareHolderAddress)
        public
        onlyOwner
    ...
    */
    function findAvailablePersonMatrix1() internal returns (uint256) {
        uint256 _referrerID;
        uint256 _referralLength = users[userList[availablePersonID]]
            .referralMatrix1
            .length;
    ...
    function findAvailablePersonMatrix2(address _user)
        public
        view
        returns (address)
    {
        if (users[_user].referralMatrix2.length < REFERRER_1_LEVEL_LIMIT)
            return _user;
        ...
        for (uint256 i = 0; i < 1022; i++) {
            if (
                users[referrals[i]].referralMatrix2.length ==
                REFERRER_1_LEVEL_LIMIT
            ) {
                if (i < 510) {
    ...
    */
    function buyLevelMatrix1(uint256 _level) public payable {
        ...
        if (_level == 1) {
            ...
            else users[msg.sender].levelExpiredMatrix1[1] = now + PERIOD_LENGTH;
            ...
            for (uint256 l = _level - 1; l > 0; l--)
                require(
                    users[msg.sender].levelExpiredMatrix1[l] >= now,
                    "Buy the previous level"
                );

            if (
                users[msg.sender].levelExpiredMatrix1[_level] == 0 ||
                now > users[msg.sender].levelExpiredMatrix1[_level]
            )
                users[msg.sender].levelExpiredMatrix1[_level] =
                    now +
                    PERIOD_LENGTH;
    ...
    /**
        ...
        * @param _level level which a user wants to buy
    */
    function buyLevelMatrix2(uint256 _level) public payable {
        require(users[msg.sender].isExist, "User not exist");
        require(
            users[msg.sender].referralCounter >= 2,
            "Need atleast 2 direct referrals to activate Team Matrix"
        );
        require(_level > 0 && _level <= 6, "Incorrect level");
        ...
        if (_level == 1) {
            ...
            else users[msg.sender].levelExpiredMatrix2[1] = now + PERIOD_LENGTH;
            ...
            for (uint256 l = _level - 1; l > 0; l--)
                require(
                    users[msg.sender].levelExpiredMatrix2[l] >= now,
                    "Buy the previous level"
                );

            if (
                users[msg.sender].levelExpiredMatrix2[_level] == 0 ||
                now > users[msg.sender].levelExpiredMatrix2[_level]
            )
                users[msg.sender].levelExpiredMatrix2[_level] =
                    now +
                    PERIOD_LENGTH;
    ...
    function payForLevelMatrix1(uint256 _level, address _user) internal {
        ...
        if (userID > 0 && userID <= 63) {
            for (uint256 i = 0; i < shareHolders.length; i++) {
                address(uint160(shareHolders[i])).transfer(
                    LEVEL_PRICE[_level] / (shareHolders.length)
                );
            ...
            if (
                users[actualReferer].levelExpiredMatrix1[_level] >= now &&
                users[actualReferer].referralCounter >= 2
            ) {
                sent = address(uint160(actualReferer)).send(
                    LEVEL_PRICE[_level]
                );
                if (sent) {
                    emit getMoneyForLevelEvent(
    ...
    function payForLevelMatrix2(uint256 _level, address _user) internal {
        address actualReferer;
        address tempReferer1;
        address tempReferer2;
        uint256 userID;
     ...
     * @return 2 branches
     */
    function viewUserReferralMatrix1(address _user)
        public
        view
        returns (address[] memory)
    {
     ...
     * @return 2 branches
     */
    function viewUserReferralMatrix2(address _user)
        public
        view
        returns (address[] memory)
    {
     ...
     */
    function viewUserLevelExpiredMatrix1(address _user, uint256 _level)
        public
        view
        returns (uint256)
    {
     ...
     * @return unix timestamp
     */
    function viewUserLevelExpiredMatrix2(address _user, uint256 _level)
        public
        view
        returns (uint256)
    {
